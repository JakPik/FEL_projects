...
int *matrix_subtract(int *matrix_A, int *matrix_B, int idx);
// Function that subtracts two matrixes
...
int main(void) {
  ...
  while (open_input == 1) {
    if (created_matrixes++ > number_of_matrixes) {
      ...
      matrix_array =
          reallocation_matrix_array(*&matrix_array, number_of_matrixes, created_matrixes);

      operations = reallocation_operation(*&operations, number_of_matrixes, created_matrixes);
    }
    input_test(scanf("%d", &row));
    input_test(scanf("%d", &column));
    matrix_array[idx] = matrix_make(row, column);
    getchar();
  ...
  int *final_matrix =
      matrix_calculation(*&matrix_array, *&operations, number_of_matrixes);
...
int **reallocation_matrix_array(int **matrix_array, int number_of_matrixes, int used) {
  int **new_matrix =
...
int *matrix_multiplication(int *matrix_A, int *matrix_B, int idx) {
  ...
  clear_matrix_memory(*&new_matrix);
  ...
  for (int i = 0; i < matrix_A[0]; i++) {
    for (int j = 0; j < matrix_A[1]; j++) {
      new_matrix[i * matrix_A[1] + j + 2] +=
          matrix_A[i * matrix_A[1] + j + 2] + matrix_B[i * matrix_A[1] + j + 2];
    }
  }
  free(matrix_A);
  matrix_A = NULL;
...
int *matrix_calculation(int **matrix_array, char *operations,
  ...
  for (int op_idx = operations_size; op_idx >= 0; op_idx--) {
  ...
  if (matrix_array_size == 0) {
    // Cechks if all operations were done base on remaining matrixes
  ...
  for (int op_idx = operations_size; op_idx >= 0; op_idx--) {
    ...
    if (operations[op_idx] == SUBTRACT) {
      matrix_compatibility(matrix_array[op_idx], matrix_array[op_idx + 1],
          SUBTRACT);

      matrix_array[op_idx] = matrix_subtract(matrix_array[op_idx],
                                             matrix_array[op_idx + 1], op_idx);
