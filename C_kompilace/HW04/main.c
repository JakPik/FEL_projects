...
int Rewrite_array(int limit, int size, int pole[limit + 1], int prvocisla[size + 1]);
int input_check(long input, int test, int err);
void print_line(long input);
long prime_decomposition_print(int divider, long *number);
...
void prime_number_decomposition(long input, int size, int pole[size + 1]) {
  ...
  for (int i = 0; i < size; i++) {
    number = prime_decomposition_print(pole[i], &number);
    if (number == 1)
      break;
    else
